...
def rollout(brain, est, model, sense_steps, seed, shot, condition="both"):
    ...
    return world.result or "GOAL", tr
...
def evaluate(brain, est, model, sense_steps, shots, condition="both"):
    ...
    for seed, shot in shots:
        r, tr = rollout(brain, est, model, sense_steps, seed, shot, condition)
    ...
    def axis(idx, keys):
        ...
        return o
    return dict(overall=round(s / n, 3), n=n,
                ...
                movement={k: round(float(np.mean([m[k] for m in ms])), 3)
                          for k in ("peak_lat", "peak_vert", "takeoff", "hop", "diag", "onset")})


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--policy", default="arcade_early_intent_ei_rl.npz")
    ...
    absent = sum(int(SV.simulate_absent_keeper(ArcadeGoalkeeperWorld(seed=s), sh)[0] == "SAVE")
                 for s, sh in shots)
    print(f"[eval] {len(shots)} HELD-OUT shots; natural-miss saves {absent}/{len(shots)}")
    ...
    try:
        ...
        result["both"] = r
        print(f"  BOTH: {r['overall']:.1%}  by_group={r['by_group']}  by_height={r['by_height']}")
        print(f"        takeoff_by_h={r['takeoff_by_height']} mean_uvert_by_h={r['mean_uvert_by_height']} "
              f"peakz_by_h={r['peakz_by_height']}")
        print(f"        movement={r['movement']}")
        print(f"        3x3={r['by_cell']}")
        if a.ablations:
            for cond in ("left_blind", "right_blind", "both_blind"):
                rc = evaluate(brain, est, model, a.sense_steps, shots, condition=cond)
                result[cond] = dict(overall=rc["overall"], by_group=rc["by_group"])
                print(f"  {cond}: {rc['overall']:.1%} by_group={rc['by_group']}")
    finally:
        brain.close()

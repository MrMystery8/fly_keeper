...
def run(controller_name,episodes,seed,telemetry=None,condition='normal',group='all'):
 env=FlyKeeper(seed);brain=MaleCNSBrain();ids=[int(brain._brain.ids[i]) for i in brain._brain.retina];ctrl=MaleCNSGoalkeeper() if controller_name=='malecns' else RandomGoalkeeper(seed);rows=[];decisions=[];start=time.perf_counter()
 ...
 saves=sum(r['result']=='SAVE' for r in rows);return rows,decisions,{'controller':controller_name,'condition':condition,'group':group,'episodes':episodes,'saves':saves,'goals':episodes-saves,'save_rate':saves/episodes,'wall_seconds':time.perf_counter()-start,'brain_ms':brain._brain.sim_ms}
def main():
 p=argparse.ArgumentParser();p.add_argument('--headless',action='store_true');p.add_argument('--visualizer',action='store_true');p.add_argument('--episodes',type=int,default=100);p.add_argument('--seed',type=int,default=7);p.add_argument('--controller',choices=['malecns','random'],default='malecns');p.add_argument('--condition',choices=CONDITIONS,default='normal');p.add_argument('--group',choices=['all','left','center','right'],default='all');a=p.parse_args();t=TelemetryServer() if a.visualizer else None
 if t:t.start()
 rows,decisions,summary=run(a.controller,a.episodes,a.seed,t,a.condition,a.group);out=ROOT/'workspace/outputs/flykeeper'/uuid.uuid4().hex;out.mkdir(parents=True);(out/'summary.json').write_text(json.dumps(summary,indent=2));(out/'config.yaml').write_text((ROOT/'workspace/experiments/flykeeper/config.yaml').read_text());(out/'run.log').write_text(json.dumps(summary)+'\n')
